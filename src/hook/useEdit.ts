...
export default function useEdit(option: Option) {
  ...
  const editText = computed(() => `${defaultText}-${props.flag === flagType.add ? '新增' : '编辑'}`)

  watch(
    () => props.flag,
    () => {
      state.visible = [flagType.add, flagType.edit, flagType.detail].includes(props.flag)
...
// TODO: 自动生成规则
export function defaultRule(key: string, labelMap: any, needTrigger = true) {
  const rule: Rule = {
    required: true,
